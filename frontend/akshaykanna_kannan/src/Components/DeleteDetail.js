...
class DeleteDetail extends React.Component {
    ...
    delete = (e) => {
        axios({
            method: 'get',
            url: `http://localhost:3007/user/delete/${this.props.match.params.id}`,
        })
            .then((response) => {
                const obj = {
        ...
        return (
            <div>
                ...
                <div className="container" style={{ fontFamily: "'Work Sans', sans-serif" }}>
                    <Link to="/body_service">  <button type="button" className="btn btn-primary btn-block" onClick={this.delete}>Delete the User</button></Link>
                </div>
            </div>
        )
    }
